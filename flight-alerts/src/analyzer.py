...
def check_for_deals(fares: list[dict], route: dict, departure_date) -> list[dict]:
    ...
    for fare in fares:
        ...
        if typical_low is not None and price < typical_low * DEAL_THRESHOLD:
            ...
            median, day_count = db.get_median_price(
                fare["origin"], fare["destination"], departure_date
            )
            if (
                median is not None
                and day_count >= MIN_HISTORY_DAYS
                and price < median * DEAL_THRESHOLD
            ):
        ...
        if db.was_recently_alerted(fare["origin"], fare["destination"], departure_date):
            log.info(
                "Skipping %s on %s — already alerted in cooldown window",
                route["label"],
                departure_date,
            )
            continue
